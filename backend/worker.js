          ...
          try {
            ...
            if (scored.score >= 80) {
              ...
              if (!calledTokens.has(scored.addr)) {
                // ── Dip & Recovery filter ──
                // Au lieu de call immédiat : on attend de voir si le token dip puis rebondit
                const watch = watchPending.get(scored.addr);
                const currentMcap = scored.mcap;

                if (!watch) {
                  // Premier passage : mise en watchlist
                  ...
                  const recoveryFromDip = watch.dipReached
                    ? (currentMcap - watch.minMcap) / watch.minMcap
                    : 0;
                  const waitMs = now - watch.firstSeenAt;

                  // REJECT : dump >= 40% depuis la première détection → on passe
                  if (dropFromFirst >= 0.40) {
                    watchPending.delete(scored.addr);
                    console.log(`[Worker] REJECT DIP: ${scored.symbol} dump ${(dropFromFirst*100).toFixed(0)}% depuis détection`);
                  }
                  // CALL : dip 10%+ puis rebond 10%+ → bonne entrée
                  ...
                  // CALL : timeout 3 min sans dump majeur → call normal
                  else if (waitMs >= 3 * 60 * 1000) {
                    watchPending.delete(scored.addr);
                    console.log(`[Worker] CALL TIMEOUT: ${scored.symbol} confirmé après ${Math.round(waitMs/1000)}s`);
                    // ↓ call
                  }
                  else {
                    // Toujours en attente
                    console.log(`[Worker] WATCHING: ${scored.symbol} peak=$${watch.peakMcap} min=$${watch.minMcap} cur=$${currentMcap} dip=${(dipFromPeak*100).toFixed(0)}%`);
                    try { await saveCall({ addr: scored.addr, score: scored.score, mcap: scored.mcap, liq: scored.liq, debug: scored.debug }); } catch(e) {}
                    // On break ici pour ne pas call
                    return; // eslint-disable-line no-useless-return
                  }

                  // Si on arrive ici : on call
